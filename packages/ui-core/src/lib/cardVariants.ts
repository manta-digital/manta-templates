...
export const cardVariants = cva(
  'transition-all duration-300 ease-in-out p-8',
  {
    variants: {
      variant: {
        primary:
          'bg-[var(--color-card-accent)] text-[var(--background)] border border-[var(--color-card-border)]',
        default:
          'bg-card text-card-foreground border border-[var(--color-card-border)] shadow-sm',
        bordered:
